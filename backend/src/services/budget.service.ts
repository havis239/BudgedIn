...
import { budgetRepository, type BudgetInput, type BudgetQuery, type BudgetUpdateInput } from '../repositories/budget.repository.js';
...
export const budgetService = {
  async list(userId: string, query: BudgetQuery) {
  ...
  async evaluateNotifications(userId: string, month: number, year: number) {
...
export const getBudgetStatus = (percentage: number) => {
  ...
  if (percentage >= 81) return 'Critical';
